...
CUPS_ENV_HOST="$(jq --raw-output '.CUPS_ENV_HOST' $CONFIG_PATH)"
CUPS_ENV_PASSWORD="$(jq --raw-output '.CUPS_ENV_PASSWORD' $CONFIG_PATH)"
CUPS_ENV_DEBUG="$(jq --raw-output '.CUPS_ENV_DEBUG' $CONFIG_PATH)"
...
# Password check.. 
if [ ${CUPS_ENV_PASSWORD} = "changeme" ]; then RETURN=1; REASON="Change password for root user!"; exit; fi
